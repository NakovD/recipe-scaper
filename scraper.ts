...
async function main(): Promise<void> {
  ...
  while (true) {
    ...
    // Вземи всички линкове, които са рецепти (не са /brutalni-recepti/)
    const links: string[] = await page.$$eval("a[href]", (anchors) =>
      anchors
        .map((a) => (a as HTMLAnchorElement).href)
        .filter(
          (href) =>
            href.includes("brutalnovkusno.bg") &&
            !href.includes("/brutalni-recepti") &&
            !href.includes("/wp-") &&
            href !== "https://brutalnovkusno.bg/" &&
            href.startsWith("https://brutalnovkusno.bg/")
        )
    );
    ...
    newLinks.forEach((l) => allRecipeLinks.add(l));
    console.log(`  Намерени: ${newLinks.length} нови (общо: ${allRecipeLinks.size})`);
    break;
    currentPage++;
    await sleep(1000);
  }
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (fs.existsSync(filePath)) {
      console.log(`  [${i + 1}/${links.length}] Пропускам (вече съществува): ${slug}`);
      continue;
    }
